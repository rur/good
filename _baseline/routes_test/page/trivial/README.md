...
```
good page ./baseline/routes_test trivial
```
...
- It is used to generate the _routes.go_ code with the `good routes ./baseline/routes_test trivial` command
...
Within the trivial page package the `resources.go` file load a set of per-request resources, like user details
